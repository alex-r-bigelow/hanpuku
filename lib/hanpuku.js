...
(function () {
    ...
    function standardizeText(e, m) {
        var newAnchor,
            ...
            oldAnchor_x = e.getAttribute('x') === null ? 0 : parseFloat(e.getAttribute('x')),
            oldAnchor_y = e.getAttribute('y') === null ? 0 : parseFloat(e.getAttribute('y'));
        
        // Like reverseTransform, we need to store the old relative
        // anchor, so we can restore it when we come back
        e.setAttribute('hanpuku_oldInternalX', oldAnchor_x);
        e.setAttribute('hanpuku_oldInternalY', oldAnchor_y);
        
        // Flatten the x and y coordinates into the matrix
        newAnchor = matPoint(m, oldAnchor_x, oldAnchor_y);
        m[4] = newAnchor[0];
        m[5] = newAnchor[1];
        ...
        // flattened chain of ancestral matrices, plus
        // the flattened internal x,y coordinate
        ...
        if (transform !== null) {
            ...
            for (i = 0; i < transform.length; i += 1) {
                ...
                if (stringMode === 'translate') {
                    ...
                    reverseTransform += 'scale(' + 1/params[0] + ',' + 1/params[1] + ')';
                    m = matMultiply(m, [Number(params[0]),0,0,Number(params[1]),0,0]);

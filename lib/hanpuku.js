...
(function () {
    ...
    var pathSplitter = new RegExp('[MmZzLlHhVvCcSsQqTtAa]', 'g'),
        magicCircleNumber = 4*(Math.sqrt(2)-1)/3;
    ...
    function getArrayOfNumbers(mainArray, startIndex, stopIndex, m, offsets) {
        ...
        for (i = startIndex; i <= stopIndex; i += 2) {
            newPoint = matPoint(m, offsets[0] + Number(mainArray[i]), offsets[1] + Number(mainArray[i+1]));
    ...
    function arcToCubic(A, rx, ry, theta, longflag, sweepflag, B) {
        // This code adapted from http://fossies.org/dox/inkscape-0.48.5/cubicsuperpath_8py_source.html
        // (my maths kind of suck)
        ...
        // Convert to a unit circle
        var mat = matMultiply([1/rx, 0, 0, 1/ry, 0, 0],
                              [-cosTheta, -sinTheta, sinTheta, -cosTheta, 0, 0]);
        mat = matMultiply([cosTheta, sinTheta, -sinTheta, cosTheta, 0, 0], mat);
        
        A = matPoint(mat, A[0], A[1]);
        B = matPoint(mat, B[0], B[1]);
        
        var k=[-(B[1]-A[1]), B[0]-A[0]];
        var d = k[0]*k[0] + k[1]*k[1];
        k[0] = k[0] / Math.sqrt(d);
        k[1] = k[1] / Math.sqrt(d);
        ...
        var O=[(B[0]+A[0])/2 + d*k[0],
               (B[1]+A[1])/2 + d*k[1]];
        var OA=[A[0]-O[0], A[1]-O[1]];
        var OB=[B[0]-O[0], B[1]-O[1]];
        
        var start = Math.acos(OA[0] / Math.sqrt(OA[0]*OA[0] + OA[1]*OA[1]));
        if (OA[1] < 0) {
            start = -start;
        }
        
        var end = Math.acos(OB[0] / Math.sqrt(OB[0]*OB[0] + OA[1]*OA[1]));
        if (OB[1] < 0) {
            end = -end;
        }
        
        if (sweepflag === '1' && start > end) {
            end += 2 * Math.PI;
        }
        ...
        var NbSectors = Math.floor(Math.abs(start-end)*2/Math.PI)+1;
        var dtheta = (end-start) / NbSectors;
        
        var v = 4*Math.tan(dtheta/4)/3,
            v1,
            v2,
            pt,
            i;
        ...
        for (i = 0; i <= NbSectors; i += 1) {
            angle = start + i*dtheta;
            v1 = [O[0] + Math.cos(angle) + v * Math.sin(angle),
                  O[1] + Math.sin(angle) - v * Math.cos(angle)];
            pt = [O[0] + Math.cos(angle),
                  O[1] + Math.sin(angle)];
            v2 = [O[0] + Math.cos(angle) - v * Math.sin(angle),
                  O[1] + Math.sin(angle) + v * Math.cos(angle)];
            
            if (i > 0) {
                // the path string will already have the first pt, and we won't need / can't use v1
                result.push(v1);
                result.push(pt);
            }
            if (i < NbSectors) {
                // only start a C command if we'll finish it; otherwise we don't need / can't use v2
                result.push('C');
                result.push(v2);
            }
        }
        ...
        for (i = 0; i < result.length; i += 1) {
            if (typeof result[i] !== 'string') {
                result[i] = matPoint(mat, result[i][0], result[i][1]);
            }
        }
        return result;
    }
    function pathToCubicPath(d, m) {
            ...
            if (dParts[i] === 'C' || dParts[i] === 'c') {
                ...
                for (j = 0; j < temp.length; j += 1) {
                    if (typeof temp[j] === 'string') {
                        result[currentSegment].push(temp[j]);
                    } else {
                        result[currentSegment].push(temp[j][0]);
                        result[currentSegment].push(temp[j][1]);
                    }
                }
                
                i += 8;
                lastAnchor = lastCubicControl = temp[temp.length - 1];
            } else {
                console.log(dParts, i);
        ...
        for (i = 0; i < result.length; i += 1) {
            for (j = 0; j < result[i].length; j += 1) {
                if (j > 0 && typeof result[i][j] !== 'string' && typeof result[i][j-1] !== 'string') {
                    temp += ',';
                }
                temp += result[i][j];
            }

...
(function () {
    ...
    function pathToCubicPath(d, m) {
        ...
        for (i = 0; i < pointTypes.length; i += 1) {
            if (pointTypes[i].toUpperCase() !== pointTypes[i]) {
                throw new Error('Hanpuku Error: no support for relative point ' + pointTypes[i] + ' yet');
            }
            ...
            if (i === 0) {
                if (pointTypes[i] === 'M') {
                    ...
                    throw new Error('Hanpuku Error: paths must start with M');
                }
                ...
                throw new Error('Hanpuku Error: no support yet for disjoint paths');
            } else {
                throw new Error('Hanpuku Error: no support yet for path point ' + pointTypes[i]);
            }
        }
    ...
    function standardizeText(e, m) {
        ...
        textAnchor = [e.getAttribute('x') === null ? 0 : parseFloat(e.getAttribute('x')),
                      e.getAttribute('y') === null ? 0 : parseFloat(e.getAttribute('y'))];
        
        m = matMultiply(m, [1,0,0,1,textAnchor[0],textAnchor[1]]);
        e.removeAttribute('x');
        e.removeAttribute('y');
        e.setAttribute('transform', 'matrix(' + m.join(',') + ')');
        
        // Extract translation, scale, and rotate
        textTransforms.x = m[4];
        textTransforms.y = -m[5];
        textTransforms.sx = Math.sqrt(m[0]*m[0] + m[2]*m[2]);
        textTransforms.sy = Math.sqrt(m[1]*m[1] + m[3]*m[3]);
        textTransforms.theta = Math.atan(m[1]/m[3]);
        console.log(textTransforms);
        e.setAttribute('hanpuku_textTransforms', JsonCircular.stringify(textTransforms));
    ...
    function elementToCubicPaths(e, m, preserveReverseTransforms) {
        ...
        if (transform && transform !== "null") {
            ...
            if (preserveReverseTransforms && e.tagName !== 'text') {

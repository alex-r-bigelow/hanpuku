/*jslint regexp: true*/
/*global convertUnits, d3, window, document*/
/**
...
(function () {
    ...
    function arcToCubic(A, rx, ry, theta, longflag, sweepflag, B) {
        ...
        temp = (longflag === sweepflag ? -1 : 1) *
                Math.sqrt(Math.abs((rx2 * ry2 - rx2 * vy2 - ry2 * vx2) /
                                   (rx2 * vy2 + ry2 * vx2)));
        Cprime = [temp * rx * v[1] / ry,
                 -temp * ry * v[0] / rx];
        
        // Step 3
        mat = [cosTheta, sinTheta, -sinTheta, cosTheta, 0, 0];
        C = matPoint(mat, Cprime[0], Cprime[1]);
        C[0] += (A[0] + B[0]) / 2;
        C[1] += (A[1] + B[1]) / 2;
        ...
        // Figure out the angles at which to create segments
        Aangle = Math.atan2(A[1], A[0]);
        Bangle = Math.atan2(B[1], B[0]);
        
        dTheta = Math.abs(Aangle - Bangle);
        if ((longflag === '1' && dTheta < Math.PI) ||
                (longflag === '0' && dTheta > Math.PI)) {
            dTheta = 2 * Math.PI - dTheta;
        }
        dTheta = sweepflag === '1' ? dTheta : -dTheta;
        
        numSplits = 0;
        angles = [];
        
        // TODO: in theory, segments up to pi/4 should be adequate,
        // but I don't think the handles are winding up in quite
        // the right spots... for now I split it up into 20 segments
        // so it's harder to notice
        // but pi/7 seems to be necessary (and there's a weird anomaly with pi/2)
        while (Math.abs(dTheta) > Math.PI / 20) {
            numSplits += 1;
            dTheta = dTheta / 2;
        }
        ...
        for (i = 1; i < angles.length; i += 1) {    // i = 1: don't need to re-write point A in the path commands
            ...
            // get the first and second points on the circle
            A = [Math.cos(angles[i - 1]),
                 Math.sin(angles[i - 1])];
            B = [Math.cos(angles[i]),
                 Math.sin(angles[i])];
            // get a point on the line that bisects A and B
            M = [Math.cos((angles[i] + angles[i - 1]) / 2),
            ...
            if (A[1] === 0) {   // Special cases where tangents have undefined slopes
                ...
                x = (A[1] + A0_2 / A[1] - M[1] - M0_2 / M[1]) / (A[0] / A[1] - M[0] / M[1]);
            }
            // y-coordinate
            if (A[1] === 0 && M[1] === 0) {
                y = 0;
            } else if (A[1] !== 0) {
                y = -A[0] * x / A[1] + A[1] + A0_2 / A[1];
            } else {
                y = -M[0] * x / M[1] + M[1] + M0_2 / M[1];
            }
            ...
            if (B[1] === 0) {   // Special cases where tangents have undefined slopes
                ...
                x = (B[1] + B0_2 / B[1] - M[1] - M0_2 / M[1]) / (B[0] / B[1] - M[0] / M[1]);
            }
            // y-coordinate
            if (B[1] === 0 && M[1] === 0) {
                y = 0;
            } else if (B[1] !== 0) {
                y = -B[0] * x / B[1] + B[1] + B0_2 / B[1];
            } else {
                y = -M[0] * x / M[1] + M[1] + M0_2 / M[1];
            }
    ...
    function pathToCubicPath(d, m) {
        ...
        while (i < dParts.length) {
            ...
            if (dParts[i] === 'C' || dParts[i] === 'c') {
                ...
                temp = getArrayOfNumbers(dParts, i + 6, i + 7,
    ...
    function lineToCubicPath(x1, y1, x2, y2, m) {
        return pathToCubicPath("M" + x1 + "," + y1 +
                               "L" + x2 + "," + y2,
                               m);
    }
    
    function rectToCubicPath(x, y, width, height, m) {
        return pathToCubicPath("M" + x + "," + y +
                               "L" + (x + width) + "," + y +
                               "L" + (x + width) + "," + (y + height) +
                               "L" + x + "," + (y + height) +
                               "L" + x + "," + y +
    ...
    function circleToCubicPath(cx, cy, r, m) {
        return pathToCubicPath("M" + cx + "," + (cy - r) +    // top
    ...
    function getCoordinate(e, attr) {
        return e.hasAttribute(attr) ? parseFloat(convertUnits(e, e.getAttribute(attr), 'px')) : 0;
    }
    ...
    function standardizeText(e, m) {
        ...
        // (I break this into two lines to keep jslint quiet)
        content = e.textContent;
        e.textContent = content;
    }
    
    function convertNodeToPath(e, d) {
    ...
    function elementToCubicPaths(e, m, preserveReverseTransforms) {
        ...
        if (transform !== null && !e.hasAttribute('hanpuku_nonNativeTransform')) {
            if (e.hasAttribute('hanpuku_prependTransform')) {
                transform = e.getAttribute('hanpuku_prependTransform') + ' ' + transform;
                e.removeAttribute('hanpuku_prependTransform');
            }
